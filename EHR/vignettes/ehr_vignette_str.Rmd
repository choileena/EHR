...
The data for example 1 includes a demographic file, a concentration file, an intravenous (IV) dosing file, and a lab file, which are all cleaned and formatted appropriately. We also define a directory for the raw data and a directory for interactive checking output files.
...
The function `pullRealId()` replaces the module IDs -- `mod_id`, `mod_visit`, and `mod_id_visit` -- with the original IDs -- `subject_id` and `subject_uid`.
