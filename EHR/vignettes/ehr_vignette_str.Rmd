...
The `EHR` package provides several modules to perform diverse medication-related studies using data from electronic health record (EHR) databases. Especially, the package includes modules to perform pharmacokinetic/pharmacodynamic (PK/PD) analyses using EHRs, as outlined in Choi, et al.$^{1}$, and additional modules will be added in future. This vignette describes four modules for processing data (*Pro-Demographic*, *Pro-Med-Str*, *Pro-Drug Level*, *Pro-Laboratory*) and one module for PK data building (*Build-PK-IV*) in the system, when data are typically obtained from a structured database.

The process starts with structured data extracted by Structured Query Language (SQL) from EHRs or provided by a user, then moves through two phases: data processing which standardizes and combines the input data (*Pro-Med-Str*, *Pro-Drug Level*, etc.) and data building which creates the final PK/PD data (*Build-PK-IV*).
...
To begin we load the `EHR` package, the `pkdata` package (which can be installed from github), and the `lubridate` package. 

```{r load-lib-dir, eval=FALSE}
# to install pkdata package
# library(devtools)
# install_github("couthcommander/pkdata")
...
The data for example 1 includes a demographic file, a concentration file, an IV dosing file, and a lab file, which are all cleaned and formatted appropriately. We also define a directory for the raw data and a directory for interactive checking output files.

```{r simp-in, cache=TRUE}
# define directories
rawDataDir <- system.file(file.path("examples","str_ex1"), package="EHR")
checkDir <- system.file(file.path("examples","str_ex1","check"), package="EHR")
...
The `EHR` package modules use a standardized naming convention for patient identification (ID) variables. We rename the unique patient-level ID from `patient_id` to `mod_id` and the visit-level ID from `patient_visit_id` to `mod_id_visit`.
...
Next, we perform a few modifications to the datasets before building the final PK data. The demographic data is formatted as a list with two named entries. The first - called 'demo' - is a data.frame containing the demographic data; the second - called 'exclude' - is a vector of visit IDs (`mod_id_visit`) to be excluded from the final data, if any. We use the `lubridate` function `ymd_hms()` to convert the character date.time variable in the creatinine lab data to POSIXct format. 

```{r simp-mods, cache=TRUE}
demo.data <- list(demo=demo, exclude=NULL)
creat.data$date.time <- lubridate::ymd_hms(creat.data$date.time)
```

*! NB: a crosswalk is necessary for run_Build_PK_IV() to work correctly*

```{r simp-xwalk, cache=TRUE}
...
Using the four datasets, we can build a final PK dataset with the function `run_Build_PK_IV()`. Additional details for this function are provided in *Build-PK-IV* of the Example 2: Complete Data Processing and Building from Raw Extracted Data to PK Data section below.

```{r sim-build-pk-iv, cache=TRUE}
...
```{r ex2-dirs}
rawDataDir <- system.file(file.path("examples","str_ex2"), package="EHR")
dataDir <- system.file(file.path("examples","str_ex2"), package="EHR")
checkDir <- system.file(file.path("examples","str_ex2","check"), package="EHR")
```
...
Each raw dataset should contain a subject visit id, a subject unique id, or both ids. In this example the subject visit id is called `subject_id` and the subject unique id is called `subject_uid`. The subject visit id is combination of subject and course -- e.g., `subject_id` 14.0 is the first course for subject 14, `subject_id` 14.1 is the second course for subject 14, etc.. `subject_uid` is a unique ID that is the same across all subject records -- the `subject_uid` 62734832 is associated with both `subject_id` 14.0 and `subject_id` 14.1.

### (1) Read and clean raw data

The demographics data file contains both ID variables `subject_id` and `subject_uid`, in addition to demographic variables such as gender, date of birth, height, weight, etc. The demographics csv file is read in using the `readTransform()` function. 
...
The sampling times data csv file can be read in with `read.csv()`. The function `dataTransformation()` is then used to rename the variable `Study.ID` to `subject_id` and to create a new variable called `samp` using the `modify=` argument. 
...
The example drug dosing data consists of files FLOW_DATA.csv and MAR_DATA.csv containing two sources of dose information. The FLOW data csv file contains aliases for both ID variables; it is read in with the `readTransform()` function which renames the variables `Subject.Id` to `subject_id` and `Subject.Uniq.Id` to `subject_uid`. 
...
The MAR data csv file contains several variables with a colon (:) character. To preserve the colon in the variable name, the data should be read in without checking for syntactically valid variable names. The data is read in using `read.csv()` with the argument `check.names = FALSE` and then passed to the `dataTransformation()` function which renames `Uniq.Id` to `subject_uid`.
...
creat.in <- readTransform(file.path(rawDataDir, "Creatinine_DATA.csv"), rename = c('Subject.uniq' = 'subject_uid'))
head(creat.in)

# Albumin lab data
alb.in <- readTransform(file.path(rawDataDir, "Albumin_DATA.csv"), rename = c('Subject.uniq' = 'subject_uid'))
head(creat.in)
```
...
The `dat=` argument should contain the cleaned input data.frame from step (1) and the `xwalk=` argument should contain the crosswalk data.frame produced in step (2). Additional arguments `firstCols=` and `orderBy=` control which variables are in the first columns of the output and the sort order, respectively. The cleaned structured data are saved as R objects for use with the modules.

```{r mod-id-data, cache=TRUE}
## demographics data
demo.cln <- pullFakeId(demo.in, mod.id, firstCols = c('mod_id', 'mod_visit', 'mod_id_visit'), uniq.id = 'subject_uid')
head(demo.cln)
saveRDS(demo.cln, file=file.path(dataDir,"Fentanyl_demo_mod_id.rds"))

## drug level data
# sampling times
samp.cln <- pullFakeId(samp.in, mod.id, firstCols = c('mod_id', 'mod_visit', 'mod_id_visit', 'samp'), 
...
conc.cln <- pullFakeId(conc.in, mod.id, firstCols = c('record_id', 'mod_id', 'mod_visit', 'mod_id_visit', 'samp'),
                       orderBy = 'record_id',
                       uniq.id = 'subject_uid')
head(conc.cln)
...
## dosing data
# flow
flow.cln <- pullFakeId(flow.in, mod.id, firstCols = c('mod_id', 'mod_visit', 'mod_id_visit'),uniq.id = 'subject_uid')
head(flow.cln)
...
Before running the modules, it is necessary to define several options and parameters. Using `options(pkxwalk =)` allows the modules to access the crosswalk file. We also create a `drugname` stub and define the lower limit of quantification (LLOQ) for the drug of interest.
...
The Demographic module accepts the cleaned structured demographic dataset and a user-defined set of exclusion criteria and returns a formatted list with the demographic data and records meeting the exclusion criteria suitable for integration with the other modules.
...
* The output of `run_Demo()` is a list with two components: `demo`, a dataframe containing a demogrpahic information and `exclude`, 	
vector of excluded visit IDs

## Pro-Med-Str

The Pro-Med-Str module processes structured medication data. Part I handles intravenous (IV) dose data and Part II handles e-prescription data.

### Part I: IV dose data

Part I handles IV dose data from 2 sources, Flow data generated by the infusion pump and manually recorded Medication Administration Record (MAR) data. The module is semi-interactive in the sense that it generates several files to check potential data errors and get feedback from an investigator. If corrected information ('fix' files) are provided, the module should be re-run to incorporate the corrections. The major functions of this module are:
...
* Process MAR data using the given medications in the `medchk.path` file, infusion units, bolus units, and bolus rate threshold. If there are MAR rows containing units other than those specified, a file listing these records will be produced using the `failunit_fn` stub appended with drugname
...
* Merge infusion dose data and bolus dose data and conform doses. See the `conformDoses()` documentation for additional details. 

The IV dose data can be processed by the `run_MedStrI` function using: 

```{r Pro-Med-Str1, cache=TRUE, eval=TRUE}
...
* The `run_MedStrI` function arguments are as follows:
    + `flow.path`: file path where the Flow data exist
    + `flow.select`: list of variables in the flow IV data, which are used for processing
    + `flow.rename`: rename variables for the variables in flow.select
    + `flow.mod.list`: list containing modifications to variables in flow data
    ...
    + `failflow_fn`: filename stub for invalid duplicate rows in flow data with rate of 0 check file. The stub will be prepended with the string 'fail' to create a .csv with the invalida data. The corrected data with failures replaced should use the same filename stub prepended with the string 'fix'.
    ...
    + `failnowgt_fn`: filename stub for records with missing flow weight and unit involving 'kg'
...
## Pro-Drug Level
Pro-Drug Level module processes drug concentration data that can be merged with medication dose data and other types of data. This module is semi-interactive in the sense that it generates several files while processing in order to check missing data and potential data errors, and get feedback from an investigator. If corrected information ('fix' files) are provided, the module should be re-run to incorporate the corrections. The major functions of this module are:

* Combine drug concentration data with sampling time: This step is necessary only if the drug concentration data file does not contain the sampling time (i.e., when blood samples were drawn for drug concentration measurements). When this is the case, the sampling time should be obtained from a separate data file.
...
```{r Pro-Drug-Level, cache=TRUE, eval=TRUE}
...
The output provided a warning message that 3 rows were missing concentration date. The file 'failMissingConcDate-fent.csv' contains the 3 records with missing values for the `date.time` variable.
...
After providing the updated file, the same `run_DrugLevel()` function should be re-run. The output now contains an additional message below the warning saying "fixMissingConcDate-fent.csv read with failures replaced". The conc.out data.frame also contains 3 additional rows with the corrected data

```{r Pro-Drug-Level-rerun, cache=TRUE, eval=TRUE}
conc.out <- run_DrugLevel(conc.path=file.path(dataDir,"Fentanyl_conc_mod_id.rds"),
                          conc.select=c('mod_id','mod_id_visit','samp','fentanyl_calc_conc'),
                          conc.rename=c(fentanyl_calc_conc = 'conc.level', samp= 'event'),
                          conc.mod.list=list(mod_id_event = expression(paste(mod_id_visit, event, sep = '_'))),
...
* The `run_DrugLevel` function arguments are as follows:
    ...
    + `samp.mod.list`: list containing modifications to variables in sampling time data
    ...
    + `conc.mod.list`: list containing modifications to variables in concentration data
    + `check.path`: file path where the generated files for data checking are stored, and the corresponding data files with fixed data exist
...
* The `run_Labs` function arguments are as follows:
    ...
    + `lab.mod.list`: list containing modifications to variables in laboratory data

## Build-PK-IV

The Build-PK-IV module creates PK data for IV medications. Both dose data from the *Pro-Med-Str1* module and concentration data from the *Pro-DrugLevel* module are required. Demographic data from the *Pro-Demographic* module and laboratory data from the *Pro-Laboratory* module may optionally be included. The module is semi-interactive in the sense that it generates several files to check potential data errors, and get feedback from an investigator. If corrected information (‘fix’ files) are provided, the module should be re-run to incorporate the corrections. The major functions this module performs are:
...
* The final PK data includes ID and standard NONMEM format variables:
...
```{r Build-PK-IV, cache=TRUE, eval=TRUE}
