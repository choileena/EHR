...
# date.time variable is expected in the demographic data; if date.time variable is not already in the demographic data, it could be generated using other varibles as done in this example 
z1 <- EHR:::fixDates(demo[, 'surgery_date'])
z2 <- sub("^([0-9]{2})([0-9]{2})$", "\\1:\\2", sprintf("%04d", demo[,
'time_fromor']))
demo[,'date.time'] <- pkdata::parse_dates(paste(z1, z2))
head(demo)
...
simple_pk_dat <- run_Build_PK_IV(
    ...
    dose=ivdose.data,
    dose.columns = list(id = 'mod_id', date = 'date.dose', infuseDatetime = 'infuse.time', infuseDose = 'infuse.dose', infuseTimeExact= 'infuse.time.real',  bolusDatetime = 'bolus.time', bolusDose = 'bolus.dose', gap = 'maxint', weight = 'weight'),
...
# date.time variable is expected in the demo.ingraphic data; if date.time variable is not already in the demo.ingraphic data, it could be generated using other varibles as done in this example 
...
ivdose.out <- run_MedStrI(
    ...
    flow.columns = list(id = 'mod_id', datetime = 'date.time', finalunits = 'final.units', unit = 'unit', rate = 'rate', weight = 'weight'),
...
* The `run_MedStrI` function arguments are as follows:
    ...
    + `flow.columns`: named list of variables required in flow data
    + `mar.path`: file path where the MAR data exist
    + `mar.columns`: named list of variables required in MAR data
    + `medGivenReq`: indicator if values in the MAR given column should equal "Given", or otherwise include NA
    ...
    + `demo.columns`: named list of variables required in demographic data
    + `missing.wgt.path`: file path to additional weight data
    + `wgt.columns`: named list of variables required in weight data
...
* `dat.columns`: the names of the columns to select
