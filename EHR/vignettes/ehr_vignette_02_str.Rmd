...
* The output of the `run_MedStrI` function is a dataset with processed IV dosing data with the following variables:
    ...
    + maxint: infusion recording interval (e.g., default: maxint = 15 min for MAR data; maxint = 60 min for flow data). Depending institutions, this output variable should be used with caution when running `run_Build_PK_IV` (see  *Build-PK-IV* for details).
...
* The `run_DrugLevel` function arguments are as follows:
    ...
    + `conc.select`: (required) list of variables in the drug concentration data, which are used for processing; id (e.g., 'mod_id'), sample event id within each subject id (e.g., 'samp'), and drug concentration (e.g., 'fentanyl_calc_conc') are required to be selected
    ...
    + `samp.path`: (optional) file name of data with sampling time (CSV, RData, RDS), or data.frame; if provided, 'conc.mod.list' and 'samp.mod.list' should be provided to create unique identification number for concentration samples within each subject, which is a combination of the subject id and the sample event id using the same variable name, and this variable is used to connect the concentration data and the sample data to generate the date/time variable for concentration data
...
For example of Vanderbilt University Medical Center, after *Epic* EHR system was implemented, only MAR data are collected without flow sheet data, which have infusion data recorded with any time intervals, and hence `maxint` should not be set for 'gap' in `dose.columns` argument of `run_Build_PK_IV` function.
